		...
		public function set_langID($langID)
		{
			...
			if (is_string($langID))
			{
				if ($langID)
				{
					...
					{
						preg_match_all('/(\w+)/u', $stopword_str, $matches);
						$this->stopwords = preg_replace('/\s+\|/u', '', $matches[0]);
					}
				}	
				return true;
			} else {
		...
		public function Autotag($text)
		{
			...
			for($n = 0; $n < $count; $n++)
			{
				...
				if (!$this->is_stopword($wordcomb1))
				{
                    ...
                    $wordfreq1[$wordcomb1] = (isset($wordfreq1[$wordcomb1])) ? $wordfreq1[$wordcomb1] + 1 : 1; 
					// Count per stemword the frequencies of all its (unstemmed) originals
					$stemfreq[$stemword][$wordcomb1] = (isset($stemfreq[$stemword][$wordcomb1])) ? $stemfreq[$stemword][$wordcomb1] + 1 : 1;
					...
					if ($n > 0)
					{
						...
						{
						    $wordfreq2[$wordcomb2] = (isset($wordfreq2[$wordcomb2])) ? $wordfreq2[$wordcomb2] + 1 : 1; 
						}
					}
					...
					if ($n > 1)
					{
						...
						{
						    $wordfreq3[$wordcomb3] = (isset($wordfreq3[$wordcomb3])) ? $wordfreq3[$wordcomb3] + 1 : 1; 
                            //$wordfreq3[$wordcomb3]++;
						}
					}
				}
			...
			*/
			...
			$wordfreq1 = $this->calcScore($wordfreq1, $this->weightArray[0]);
			$wordfreq2 = $this->calcScore($wordfreq2, $this->weightArray[1]);
			$wordfreq3 = $this->calcScore($wordfreq3, $this->weightArray[2]);
